<?xml version='1.0'?>
    ...
    <rx:meta-info>
      ...
      <xsl:if test="exists($title)">
        <rx:meta-field name="title" value="{$title}"/>
         ...
         <rx:meta-field name="publisher" value="zeppobox.com"/>
         <rx:meta-field name="keywords" value="Zeitversetzte Übergabe, Begleitendes Postfach, Permission Marketing"/>
         <rx:meta-field name="subject" value="Geschäftsmodell für Aussenwerber, Banken, Tankstellenpächter."/>
   ...
   <fo:block text-align="center" width="100%">
       <fo:external-graphic src="url({concat($artworkPrefix, 'Customization/OpenTopic/common/artwork/artikelbild-zeppobox.gif')})"/>              
   </fo:block>
...
<xsl:when test="@otherprops='center-small-second'">
<fo:block xsl:use-attribute-sets="center-small-second">
<xsl:apply-templates/>
</fo:block>
</xsl:when>
