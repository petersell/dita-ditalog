<?xml version='1.0'?>
    ...
    <rx:meta-info>
      ...
      <xsl:if test="exists($title)">
        <rx:meta-field name="title" value="{$title}"/>
         ...
         <rx:meta-field name="keywords" value="DITA xml, Darwin Information Typing Architecture, EPUB, Techcomm, Technical Writing, Technische Redaktion, Self Publishing "/>
         <rx:meta-field name="subject" value="Kostenlos Online-Hilfen, PDF-Dateien und EPUB-Bücher erstellen"/>
   ...
   <!-- external-graphic von front-matter.xsl --> 
        <fo:block text-align="center" width="100%">
          <fo:external-graphic src="url({concat($artworkPrefix, 'Customization/OpenTopic/common/artwork/artikelbild.gif')})"/>              
        </fo:block>
